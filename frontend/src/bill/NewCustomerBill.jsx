    ...
    <div className="agro-container">
      ...
      <form onSubmit={(e) => handleSave(e, false)} className="agro-card" style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div className="agro-card-header" style={{ borderBottom: '1px solid #f3f4f6', paddingBottom: '20px', marginBottom: '20px' }}>
          <h2 style={{ fontSize: '20px', margin: 0 }}>Customer Details & Payment</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          ...
          <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '8px', gridColumn: 'span 2' }}>
            ...
            <SearchableSelect 
              ...
              height="45px"
            />
            {/* Hidden input to ensure form validation passes if they didn't pick customer from list but typed it manually - wait, no, they MUST pick it from the list now */}
            <input 
              type="text" 
              name="customerName" 
          ...
          {/* Readonly customer ID to show selection */}
          <FormField label="Customer ID" name="customerId" value={formData.customerId} onChange={() => {}} placeholder="Auto-filled" />
        ...
        <div style={{ marginTop: '30px', padding: '24px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: '28px', fontWeight: '800', color: (Number(formData.totalBill || 0) - Number(formData.paid || 0)) > 0 ? '#ef4444' : '#10b981' }}>
