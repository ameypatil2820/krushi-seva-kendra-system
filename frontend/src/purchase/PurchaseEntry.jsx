...
const PurchaseEntry = () => {
  ...
  const handleChildChange = (id, field, value) => {
    const updatedChildren = children.map(child => {
      if (child.id === id) {
        ...
        if (field === 'quantity' || field === 'purchasePrice' || field === 'taxPercent') {
          const qty = field === 'quantity' ? parseFloat(value) || 0 : child.quantity;
          const price = field === 'purchasePrice' ? parseFloat(value) || 0 : child.purchasePrice;
  ...
  const calculateTotals = (currentChildren, discount) => {
    ...
    currentChildren.forEach(child => {
      ...
      subtotal += (parseFloat(child.quantity) || 0) * (parseFloat(child.purchasePrice) || 0);
  ...
  const handleMasterChange = (field, value) => {
    ...
    if (field === 'discount' || field === 'paidAmount') {
      ...
      updatedMaster.grandTotal = updatedMaster.subtotal + updatedMaster.totalTaxAmount - disc;
